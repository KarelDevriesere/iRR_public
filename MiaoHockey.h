...
const std::unordered_map<HAP_operator, double>Miao_weights = {{HAP_operator::InterClubSwap, 1.0/3.0}, {HAP_operator::IntraClubSwap, 1.0/3.0}, {HAP_operator::RandomSwap, 1.0/6.0}, {HAP_operator::ComplementInsertion, 1.0/6.0}, {HAP_operator::Initial, 0.0}};
...
int MainMiaoHockey(int argc, char** argv){
    ...
    cout << "Parameters SA are NOT tuned at all!!" << endl;
    cout << "If I do not update temperature, I get much better solutions.." << endl;
    cout << "So now I updated I_temp from 20 to 200 and I_accepted from 4 to 40" << endl;
    int answer = 0;
    if (argc < 2){
    ...
    */
    ...
    for (int inst: Instances){
        ...
        */
        ...
        if (!in.read_Miao_Hockey(file_path, InstancesMiaoChosen)){
        ...
        if (AlgoSelected.at(Algorithm::Meta_Miao)){
            for (int i = 0; i < NrAlgoRuns; ++i){
                ...
                assert(in.getNrTeams() != 608); // do not do Merged instance!!
                MiaoAlgo miao_algo(Miao_operators, weights, in, gen);
                ...
                miao_algo.SolveGivenSeqeuence(in, sol);
        ...
        */
        ...
        if (AlgoSelected.at(Algorithm::FO_Karel)){
            for (int i = 0; i < NrAlgoRuns; ++i){
                ...
                ReadSolution(file_path_initial_solution, sol);
                FO fo(in, FO_operators, FO_weights, gen);
                // cout << "Solve FO" << endl;
                fo.solve(in,sol);
                sol.validate();
                int MetaSeed = seed+i; // use this as identifier to store results
        ...
        */
        ...
        if (AlgoSelected.at(Algorithm::Meta_Karel)){
            ...
            for (int i = 0; i < NrAlgoRuns; ++i){
                ...
                ILS ils(Karel_operators, Karel_weights, gen);
