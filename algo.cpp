...
void GreedyPerfectMatchings(){
	...
	output_file.open("results_greedy_matching.txt");
	output_file << "Instance N R Greedy Optimal Gap \n";
	for (const auto& file: std::filesystem::recursive_directory_iterator("C:\\Users\\kardvrie\\C++\\VSprojects\\test2\\Instances\\Single\\")){
		cout << file << endl;
		...
		Solution sol(in);
		int obj_greedy = 0;
		int obj_matching_boost, obj_matching_gurobi;
		vector<vector<bool>>GameForbidden(in.getNrTeams(), vector<bool>(in.getNrTeams(), false));
		for (int r = 0; r < in.getNrRounds(); ++r){
			// obj_matching_boost = MWPM(sol, GameForbidden);
			GurSolver gur_greedy(in);
			obj_matching_gurobi = gur_greedy.PerfectMatching(GameForbidden);
			// cout << "Expected: " << obj_matching_boost << " == " << obj_matching_gurobi << endl;
			// assert(obj_matching_boost == obj_matching_gurobi);
			obj_greedy += obj_matching_gurobi;
		}

		GurSolver gur(in);
		bool HA = false;
		bool relax_x = false;
		gur.build_all(HA, relax_x); // Only travel constraints so far!!
		bool with_obj = true;
		// int obj = gur.solve(with_obj);

		// double gap = ((double)(obj_greedy - obj)/obj)*100.0;

		// output_file << in.getNrTeams() << " " << in.getNrRounds() << " " << obj_greedy << " " << obj << " " << gap << "\n";

		/*
		std::cout << "Result: " << std::endl;
		std::cout << "-----------------------" << std::endl;
		std::cout << "Greedy solution = " << obj_greedy << std::endl;
		std::cout << "Optimal solution = " << obj << std::endl;
		std::cout << "-----------------------" << std::endl;
		*/
	}
}